...
async function runWizard(opts: InitOptions): Promise<void> {
  ...
  const maxAgents = await input({
    message: "  Max concurrent agents per pipeline:",
    default: "3",
  });

  const tddEnforcement = await confirm({
    message: "  Enable TDD enforcement (RED state verification before impl)?",
    default: true,
  });
  ...
  if (shouldSetupGithub) {
    ...
    if (allRepos.length === 0) {
      ...
      for (const repoName of selectedRepoNames) {
        ...
        const statusFieldId = statusInfo?.fieldId ?? "";
  ...
  if (repos.length === 0 && shouldSetupGithub) {
  ...
  console.log("\nSetup complete! Try:\n");
  console.log("  hog demo            # See a simulated pipeline run");
  console.log("  hog pipeline create # Start an AI development pipeline");
  console.log("  hog cockpit         # Pipeline cockpit TUI");
  console.log("  hog daemon start    # Start the hogd daemon\n");
}
