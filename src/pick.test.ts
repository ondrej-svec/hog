...
function makeConfig(overrides: Partial<HogConfig> = {}): HogConfig {
  return {
    ...
    repos: [makeRepo()],
    board: { refreshInterval: 60, backlogLimit: 20, assignee: "ondrej-svec", focusDuration: 1500 },
...
function makeGitHubIssue(overrides: Record<string, unknown> = {}) {
  return {
    ...
    url: "https://github.com/aibilitycz/aibility/issues/145",
    state: "open",
...
describe("pickIssue", () => {
  ...
  it("assigns issue on GitHub and creates TickTick task", async () => {
    ...
    expect(result.issue.repo).toBe("aibilitycz/aibility");
    expect(mockAssignIssue).toHaveBeenCalledWith("aibilitycz/aibility", 145);
  ...
  it("warns if issue already assigned to self", async () => {
    const issue = makeGitHubIssue({ assignees: [{ login: "ondrej-svec" }] });
    mockFetchRepoIssues.mockReturnValue([issue]);
