...
vi.mock("./tdd-enforcement.js", () => ({
	verifyRedState: vi.fn().mockResolvedValue({ passed: true, failingTests: 3, passingTests: 0, detail: "3 tests failing (mock)" }),
}));

// Mock role-context — writeRoleClaudeMd writes to worktree directories
vi.mock("./role-context.js", () => ({
	writeRoleClaudeMd: vi.fn(),
	buildAgentLaunchArgs: vi.fn().mockReturnValue(["--dangerously-skip-permissions", "-p", "test prompt"]),
...
function createMockWorktrees(): WorktreeManager {
	return {
		...
		branchName: vi.fn().mockImplementation(
			(featureId: string, role: string) => `hog/${featureId}/${role}`,
		),
