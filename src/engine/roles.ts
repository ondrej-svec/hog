...
const BRAINSTORM_PROMPT = [
  ...
  "- What problem are we actually solving? Strip away assumptions.",
  "- Who has this problem and when? Context changes solutions.",
  "- What does success look like? Not features — outcomes.",
  "- Research the codebase: look for existing patterns, prior art, related code.",
  "- Ask ONE question at a time. Don't dump a questionnaire.",
  "",
  "### Phase 2: Explore approaches (2-3 options with tradeoffs)",
  "- For each approach: what it optimizes for, what it costs, who's done it before.",
  ...
  "",
  "### Phase 3: Converge on stories",
  "- Once the approach is clear, write user stories to tests/stories/{slug}.md",
  "- Each story needs: unique ID (STORY-001), description, acceptance criteria checklist, edge cases",
  "- Review stories with the human. Iterate until they're sharp.",
  "",
  "### Phase 4: Ship it",
  '- When the human confirms: hog pipeline create "{title}" --brainstorm-done --stories tests/stories/{slug}.md',
  "- This kicks off autonomous work: stories → tests → impl → redteam → merge",
  "",
  "## Rules",
  "- This is a CONVERSATION, not an output. Ask questions before writing anything.",
