...
const BRAINSTORM_PROMPT = [
  ...
  "",
  "- ALSO write an architecture doc to tests/stories/{slug}.architecture.md containing:",
  ...
  "  - ## File Structure: where new files go, module boundaries",
  ...
  "",
  "### Phase 4: Ship it",
  "- If advancing an existing pipeline: run `hog pipeline done {featureId}`",
  '- If creating a new pipeline: run `hog pipeline create "{title}" --brainstorm-done --stories tests/stories/{slug}.md`',
  "- This kicks off autonomous work: stories → tests → impl → redteam → merge",
  "",
  "## Rules",
...
const STORIES_PROMPT = [
  ...
  "5. Write stories to tests/stories/{slug}.md",
  "",
...
const TEST_PROMPT = [
  ...
  "If an architecture doc exists at tests/stories/{slug}.architecture.md, read it for",
  "integration patterns and dependency choices.",
  "",
  "Your job:",
...
const IMPL_PROMPT = [
  ...
  "2. **User stories** — read `tests/stories/{slug}.md` for intent and acceptance criteria",
  "3. **Architecture doc** — read `tests/stories/{slug}.architecture.md` if it exists, for integration patterns and dependency choices",
  "",
  "Your job:",
...
const REDTEAM_PROMPT = [
  ...
  "",
  "If an architecture doc exists at tests/stories/{slug}.architecture.md, read it to",
