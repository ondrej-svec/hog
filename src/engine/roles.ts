...
const STORIES_PROMPT = [
  ...
  "- Default location: `tests/stories/{slug}.md` and `tests/stories/{slug}.architecture.md`",
...
const TEST_PROMPT = [
  ...
  "",
  "Find the stories file: check `tests/stories/` first, then search the project for",
  "files matching `*{slug}*stories*.md` or `*stories*{slug}*.md`.",
  "If an architecture doc exists nearby (same directory, `.architecture.md` suffix), read it for",
  "integration patterns, dependency choices, and FILE PATHS (where to create tests).",
  "",
  "Your job:",
...
const IMPL_PROMPT = [
  ...
  "2. **User stories** — find the stories file (check `tests/stories/` or search for `*{slug}*stories*.md`)",
  "3. **Architecture doc** — find the `.architecture.md` file nearby for integration patterns,",
  "   dependency choices, and FILE PATHS (where to create source files)",
  "",
  "Your job:",
