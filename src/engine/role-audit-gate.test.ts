...
describe("roleAuditGate", () => {
  ...
  describe("impl role", () => {
    ...
    it("passes when only source files are modified", async () => {
      const result = await gate.check("/tmp", ["src/auth.ts", "src/utils/hash.ts"]);
      expect(result.passed).toBe(true);
    });
  ...
  describe("stories role", () => {
    ...
    it("passes when only docs/stories files are modified", async () => {
      const result = await gate.check("/tmp", ["docs/stories.md", "tests/stories/auth.md"]);
      expect(result.passed).toBe(true);
    });
  ...
  describe("merge role", () => {
    ...
    it("passes for any files (merge has no restrictions)", async () => {
      const result = await gate.check("/tmp", ["src/auth.ts", "src/auth.test.ts", "package.json"]);
      expect(result.passed).toBe(true);
    });
  });
